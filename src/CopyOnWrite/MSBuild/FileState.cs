...
namespace Microsoft.Build.Tasks
{
    ...
    internal class FileState
    {
        private class FileDirInfo
        {
            ...
            public FileDirInfo(string filename)
            {
                ...
                if (NativeMethods.IsWindows)
                {
                    ...
                    NativeMethods.SetThreadErrorMode(1 /* ErrorModes.SEM_FAILCRITICALERRORS */, out oldMode);
                }
                ...
                try
                {
                    if (NativeMethods.IsWindows)
                    {
                        ...
                        bool success = NativeMethods.GetFileAttributesEx(_filename, 0, ref data);
                        ...
                        if (!success)
                        {
                            ...
                            if ((error == 2 /* ERROR_FILE_NOT_FOUND */|| error == 3 /* ERROR_PATH_NOT_FOUND */)
                                && _filename.Length <= NativeMethods.MaxPath)
                            {
                                Exists = false;
                                return;
                            }
                            ...
                            NativeMethods.ThrowExceptionForErrorCode(error);
                            throw new Exception("Unreachable");
                        }

                        Exists = true;
                        IsDirectory = (data.fileAttributes & NativeMethods.FILE_ATTRIBUTE_DIRECTORY) != 0;
                        IsReadOnly = !IsDirectory
                                      && (data.fileAttributes & NativeMethods.FILE_ATTRIBUTE_READONLY) != 0;
                        LastWriteTimeUtc =
        ...
        internal FileState(string filename)
        {
            ...
            {
                throw new ArgumentException("Shared.ParameterCannotHaveZeroLength", nameof(filename));
            }
